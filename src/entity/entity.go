...
type Article struct {
	ID         int64     `json:"id" form:"id" gorm:"primary_key;column:id"`                             // 主键
	Title      string    `json:"title" form:"title" gorm:"column:title"`                                // 文章标题
	UserID     int64     `json:"userId" form:"userId" gorm:"column:user_id"`                            // 用户ID
	Content    string    `json:"content" form:"content" gorm:"column:content"`                          // 文章内容
	CreateTime time.Time `json:"createTime" form:"createTime" gorm:"column:create_time;autoCreateTime"` // 创建时间
	UpdateTime time.Time `json:"updateTime" form:"updateTime" gorm:"column:update_time;autoUpdateTime"` // 修改时间
}
